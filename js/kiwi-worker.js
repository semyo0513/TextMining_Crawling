...
/**
 * Advanced Morphological Tokenizer & Lemmatizer (+다)
 */
function advancedTokenize(text, options = {}) {
  ...
  // Remove URLs, HTML, special symbols
  ...
  for (let rawWord of words) {
    ...
    // 1. Precise Conjugation Lemmatization (+다)
    if (/(?:하고|해서|하면|하는|합니다|했어요|했습니다|하셔서|해주셔서|하지|하네요|하게|해야|함)$/.test(rawWord)) {
      ...
      // General Adjective / Verb Stem Suffix Stripping (-고, -게, -지, -면, -서, -려고, -니까)
    ...
    // 2. Auxiliary Verb Filtering (보조용언 제거)
    if (removeAuxVerbs && AUXILIARY_VERBS.has(stemmed)) {
      continue;
    }

    // 3. Conjunction & Filler Words Filtering (접속어/부사 제거)
    if (removeConjunctions && CONJUNCTIONS_AND_FILLERS.has(stemmed)) {
      continue;
    }

    // 4. Default Stopwords & Custom Stopwords Filtering
    if (DEFAULT_STOPWORDS.has(stemmed) || customStopwords.has(stemmed)) {
      continue;
    }

    // 5. Minimum Length Filter
    if (stemmed.length < minLen) {
      continue;
    }
